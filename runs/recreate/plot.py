...
def converged(data, column='ecut'):
    ...
    data["diff"] = np.abs(data["energy"] - ref_val)
    #print(data[["k-points", "converged", "diff"]])
    print(data["diff"].values[-2]/7.349988e-5)
    return data["diff"].values[-2]/7.349988e-5
...
if __name__ == '__main__':
    ...
    #                  #pd.read_csv("./")
    #                  pd.read_csv("./so_much_copper.csv")])
    ...
    data = pd.concat([data, pd.read_csv("./gga_low_kpt.csv"),
                      pd.read_csv("./just_sn_5_kpt.csv")])
    data = data.sort_values("k-points")
    formation(data)
    exit(1)
    #print(data["Element"])
    ...
    for pf in data.groupby("Element"):
        ...
        #print(e_data)
        to = converged(e_data)
        ax.scatter(e_data["k-points"], e_data["energy"])
